...
FIELD_TYPE = (
    pydantic.fields.FieldInfo if IS_PYDANTIC_V2_OR_NEWER else pydantic.fields.ModelField
)
...
def _infer_colspec_type_from_type_hint(type_hint: type[Any]) -> ColSpecType:
    ...
    if datatype := TYPE_HINTS_TO_DATATYPE_MAPPING.get(type_hint):
        ...
        if origin_type is list:
            ...
            return ColSpecType(
                dtype=Array(
                    _infer_colspec_type_from_type_hint(type_hint=internal_type).dtype
                ),
                required=True,
            )
        if origin_type is dict:
            if len(args) == 2:
                ...
                return ColSpecType(
                    dtype=Map(
                        _infer_colspec_type_from_type_hint(type_hint=args[1]).dtype
                    ),
                    required=True,
                )
            raise InvalidTypeHintException(
        ...
        if origin_type in UNION_TYPES:
            if NONE_TYPE in args:
                ...
                # Optional type
                    ...
                    effective_type = next(
                        (arg for arg in args if arg is not NONE_TYPE), None
                    )
                    return ColSpecType(
        ...
        if origin_type is list:
            ...
            if NONE_TYPE in args:
                ...
                if len(args) == 2:
                    ...
                    return _validate_data_against_type_hint(
                        data=data, type_hint=effective_type
                    )
            # Union type with all valid types is matched as AnyType
...
def _validate_list_elements(element_type: type[Any], data: Any) -> list[Any]:
    ...
    result = []
    for elem in data:
        validation_result = _get_data_validation_result(
            data=elem, type_hint=element_type
        )
    ...
    if invalid_elems:
        invalid_elems_msg = (
            f"{invalid_elems[:5]} ... (truncated)"
            if len(invalid_elems) > 5
            else invalid_elems
        )
        raise MlflowException.invalid_parameter_value(
...
def _validate_dict_elements(element_type: type[Any], data: Any) -> dict[str, Any]:
    ...
    for key, value in data.items():
        if not isinstance(key, str):
            ...
            continue
        validation_result = _get_data_validation_result(
            data=value, type_hint=element_type
        )
        if validation_result.error_message:
