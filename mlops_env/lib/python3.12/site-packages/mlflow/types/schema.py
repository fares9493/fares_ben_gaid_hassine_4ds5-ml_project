...
class Property(BaseType):
    ...
    def _merge(self, other: BaseType) -> Property:
        ...
        if isinstance(self.dtype, DataType) and isinstance(other.dtype, DataType):
            ...
            raise MlflowException(
                f"Properties are incompatible for {self.dtype} and {other.dtype}"
            )
...
class Array(BaseType):
    ...
    def _merge(self, other: BaseType) -> Array:
        ...
        if not isinstance(other, Array):
            raise MlflowException(
                f"Can't merge array with non-array type: {type(other).__name__}"
            )
        if isinstance(self.dtype, DataType):
            if self.dtype == other.dtype:
...
class Map(BaseType):
    ...
    def __init__(self, value_type: ALLOWED_DTYPES):
        ...
        if not isinstance(self._value_type, (Array, Map, DataType, Object, AnyType)):
            raise MlflowException.invalid_parameter_value(
                EXPECTED_TYPE_MESSAGE.format(
                    arg_name="value_type", passed_type=self._value_type
                )
            )
    ...
    def _merge(self, other: BaseType) -> Map:
        ...
        if not isinstance(other, Map):
            raise MlflowException(
                f"Can't merge map with non-map type: {type(other).__name__}"
            )
        if isinstance(self.value_type, DataType):
...
class ColSpec:
    ...
    def __init__(
        ...
        if not isinstance(self.type, (DataType, Array, Object, Map, AnyType)):
            raise TypeError(
                EXPECTED_TYPE_MESSAGE.format(arg_name="type", passed_type=self.type)
            )
...
class ParamSpec:
    ...
    def __init__(
        ...
        if not isinstance(self.dtype, (DataType, Object)):
            raise TypeError(
                f"'dtype' must be DataType, Object or str, got {self.dtype}"
            )
    ...
    def from_json_dict(cls, **kwargs):
        ...
        dtype = (
            Object.from_json_dict(**kwargs) if dtype == OBJECT_TYPE else DataType[dtype]
        )
        return cls(
