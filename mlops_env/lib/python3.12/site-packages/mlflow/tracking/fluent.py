...
def start_run(
    ...
    if existing_run_id:
        ...
        if parent_run_id:
            ...
            if (
                len(active_run_stack) > 0
                and parent_run_id != active_run_stack[-1].info.run_id
            ):
...
def _get_latest_active_run():
    ...
    all_active_runs = [
        run
        for run_stack in _active_run_stack.get_all_thread_values().values()
...
def autolog(
    ...
    if is_in_databricks_runtime() and (not disable):
        ...
        target_library_and_module = (
            LIBRARY_TO_AUTOLOG_MODULE | GENAI_LIBRARY_TO_AUTOLOG_MODULE
        )
    ...
    if exclude_flavors:
        ...
        target_library_and_module = {
            k: v
            for k, v in target_library_and_module.items()
            if v not in excluded_modules
        }
    ...
    def setup_autologging(module):
        try:
            ...
            autolog_module = importlib.import_module(
                target_library_and_module[module.__name__]
            )
