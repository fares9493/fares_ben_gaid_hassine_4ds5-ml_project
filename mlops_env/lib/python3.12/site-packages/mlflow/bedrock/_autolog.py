...
from mlflow.bedrock.chat import (
    convert_message_to_mlflow_chat,
    convert_tool_to_mlflow_chat_tool,
)
...
def patch_bedrock_runtime_client(client_class: type[BaseClient]):
    ...
    if hasattr(client_class, "converse_stream"):
        safe_patch(
            FLAVOR_NAME, client_class, "converse_stream", _patched_converse_stream
        )
...
def _patched_invoke_model(original, self, *args, **kwargs):
    with mlflow.start_span(name=f"{_BEDROCK_SPAN_PREFIX}{original.__name__}") as span:
        ...
        span_type = (
            SpanType.EMBEDDING if "embedding" in parsed_response_body else SpanType.LLM
        )
...
def _patched_invoke_model_with_response_stream(original, self, *args, **kwargs):
    ...
    result["body"] = InvokeModelStreamWrapper(
        stream=result["body"], client=client, span=span
    )
    return result
...
def _parse_invoke_model_response_body(
    response_body: StreamingBody,
...
def _set_tool_attributes(span, kwargs):
    ...
    if tool_config := kwargs.get("toolConfig"):
        try:
            tools = [
                convert_tool_to_mlflow_chat_tool(tool) for tool in tool_config["tools"]
            ]
