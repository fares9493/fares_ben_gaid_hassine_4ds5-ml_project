...
class InvokeModelStreamWrapper(BaseEventStreamWrapper):
    ...
    def _handle_event(self, span, event):
        ...
        self._span.add_event(
            SpanEvent(name=chunk["type"], attributes={"json": json.dumps(chunk)})
        )
...
class ConverseStreamWrapper(BaseEventStreamWrapper):
    ...
    def _handle_event(self, span, event):
        ...
        self._span.add_event(
            SpanEvent(
                name=event_name, attributes={"json": json.dumps(event[event_name])}
            )
        )
    ...
    def _close(self):
        ...
        messages = self._inputs.get("messages", []) + [
            converse_response["output"]["message"]
        ]
