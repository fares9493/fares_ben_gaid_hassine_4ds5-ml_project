    ...
    def invoke(
        ...
        for m in result["messages"]:
            try:
                ...
                messages.append(
                    parse_message(m, attachments=return_obj.get("attachments"))
                )
            except Exception:
...
class LangGraphChatAgent(ChatAgent):
    ...
    def predict(
        self,
        model_input: list[ChatAgentMessage],
        params: Optional[ChatAgentParams] = None,
    ):
        response = ChatAgentResponse(messages=[])
        for event in self.agent.stream(
            {"messages": self._convert_messages_to_dict(model_input)},
            stream_mode="updates",
        ):
