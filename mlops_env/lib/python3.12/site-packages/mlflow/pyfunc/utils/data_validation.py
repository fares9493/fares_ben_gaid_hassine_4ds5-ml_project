...
def _check_func_signature(func, func_name) -> list[str]:
    ...
    if invalid_params := set(param_names) - {
        "self",
        "context",
        "model_input",
        "params",
        ...
        warnings.warn(
            _INVALID_SIGNATURE_ERROR_MSG.format(
                func_name=func_name, invalid_params=invalid_params
            ),
            FutureWarning,
            stacklevel=2,
        )
...
def _get_func_info_if_type_hint_supported(func) -> Optional[FuncInfo]:
    ...
    if type_hint is not None:
        if _signature_cannot_be_inferred_from_type_hint(
            type_hint
        ) or _is_type_hint_from_example(type_hint):
            return
        try:
        ...
        # catch other exceptions to avoid breaking model usage
            ...
            return FuncInfo(
                input_type_hint=type_hint, input_param_name=input_param_name
            )
    else:
        color_warning(
