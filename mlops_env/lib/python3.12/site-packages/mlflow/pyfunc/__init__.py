...
def _convert_spec_type_to_spark_type(spec_type):
    ...
    if isinstance(spec_type, Map):
        return MapType(
            keyType=StringType(),
            valueType=_convert_spec_type_to_spark_type(spec_type.value_type),
        )
    ...
    for field_name in result_type.fieldNames():
        ...
        if type(field_type) in spark_primitive_type_to_np_type:
            ...
            if is_pandas_df:
                ...
                if not all(
                    _is_none_or_nan(field_value) for field_value in field_values
                ):
            ...
            if is_pandas_df:
                field_values = pandas.Series(
                    [
                        {
                            key: _convert_value_based_on_spark_type(
                                value, field_type.valueType
                            )
...
def _convert_value_based_on_spark_type(value, spark_type):
    ...
    if isinstance(spark_type, ArrayType):
        return [
            _convert_value_based_on_spark_type(v, spark_type.elementType) for v in value
        ]
...
"""
    ...
    def udf(
        iterator: Iterator[
            Tuple[Union[pandas.Series, pandas.DataFrame], ...]
        ],  # noqa: UP006
            ...
            if isinstance(input_example, list):
                messages = [
                    (
                        ...
                        else ChatMessage.from_dict(message)
                    )
            ...
            if saved_example is not None:
                ...
                try:
                    ...
                    mlflow_model.signature = _infer_signature_from_input_example(
                        saved_example,
                        _PythonModelPyfuncWrapper(
                            model_for_signature_inference, None, None
                        ),
                    )
                except Exception as e:
                    _logger.warning(
...
def update_signature_for_type_hint_from_example(
    input_example: Any, signature: ModelSignature
):
