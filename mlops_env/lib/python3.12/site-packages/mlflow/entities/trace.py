...
class Trace(_MlflowObject):
    ...
    def _repr_mimebundle_(self, include=None, exclude=None):
        ...
        if not get_display_handler().disabled:
            if is_in_databricks_runtime():
                bundle["application/databricks.mlflow.trace"] = (
                    self._serialize_for_mimebundle()
                )
    ...
    def _deserialize_json_attr(self, value: str):
        try:
            ...
            _logger.debug(
                f"Failed to deserialize JSON attribute: {value}", exc_info=True
            )
            return value
    ...
    def search_spans(
        ...
        return [
            span for span in self.data.spans if _match_name(span) and _match_type(span)
        ]
