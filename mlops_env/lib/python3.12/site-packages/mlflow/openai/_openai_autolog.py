...
def _try_parse_raw_response(response: Any) -> Any:
    ...
    try:
        ...
        _logger.debug(
            "Failed to import `LegacyAPIResponse` from `openai._legacy_response`"
        )
        return response
    ...
    if isinstance(response, LegacyAPIResponse):
        try:
            ...
            _logger.debug(
                f"Failed to parse {response} (type: {response.__class__}): {e}"
            )
...
def patched_call(original, self, *args, **kwargs):
    ...
    run_id = (
        active_run.info.run_id if active_run else getattr(self, "_mlflow_run_id", None)
    )
        ...
        if config.log_traces and request_id:
            try:
                ...
                mlflow_client.end_span(
                    request_id, span.span_id, status=SpanStatusCode.ERROR
                )
            ...
            for i, chunk in enumerate(stream):
                ...
                span.add_event(
                    SpanEvent(
                        ...
                        attributes={
                            chunk.__class__.__name__: json.dumps(
