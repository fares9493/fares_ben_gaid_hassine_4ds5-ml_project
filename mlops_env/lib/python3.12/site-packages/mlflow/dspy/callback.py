...
class MlflowCallback(BaseCallback):
    ...
    def _extract_messages_from_lm_inputs(
    ...
    def _extract_messages_from_lm_outputs(
        ...
        return [
            {
                "role": "assistant",
                "content": o.get("text") if isinstance(o, dict) else o,
            }
            for o in outputs
        ]
    ...
    def _end_span(
        ...
        if not st.span:
            _logger.warning(
                f"Failed to end a span. Span not found for call_id: {call_id}"
            )
            return
