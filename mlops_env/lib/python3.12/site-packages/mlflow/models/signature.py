...
def infer_signature(
    ...
    for key, data in schemas.items():
        if data is not None:
            try:
                schemas[key] = (
                    convert_dataclass_to_schema(data)
                    if is_dataclass(data)
                    else _infer_schema(data)
                )
...
def _infer_signature_from_type_hints(
    ...
    try:
        ...
        warnings.warn(
            f"Failed to infer signature from type hint: {e.message}", stacklevel=3
        )
        return None
    ...
    if input_example is not None:
        ...
        if not _pyfunc_decorator_used and (
            ...
            _logger.info(
                "Running the predict function to generate output based on input example"
            )
            try:
