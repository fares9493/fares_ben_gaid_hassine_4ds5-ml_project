...
def _get_mlflow_model_input_example_dict(
    ...
    return json.loads(
        _read_file_content(
            uri_or_path, mlflow_model.saved_input_example_info[INPUT_EXAMPLE_PATH]
        )
    )
...
def _enforce_map(data: Any, map_type: Map, required: bool = True):
    if (not required or isinstance(map_type.value_type, AnyType)) and (
        data is None or data == {}
    ):
        return data
    ...
    return {
        k: _enforce_type(v, map_type.value_type, required=required)
        for k, v in data.items()
    }
...
def _validate_and_get_model_code_path(model_code_path: str, temp_dir: str) -> str:
    ...
    if not (model_code_path.exists() or _databricks_path_exists(model_code_path)):
        additional_message = (
            f" Perhaps you meant '{model_code_path}.py'?"
            if not model_code_path.suffix
            else ""
        )
