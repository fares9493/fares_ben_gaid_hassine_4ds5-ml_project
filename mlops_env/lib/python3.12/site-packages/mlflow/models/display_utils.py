...
def _is_output_chat_completion_response(outputs: schema.Schema) -> bool:
    ...
    content = next(
        filter(lambda prop: prop.name == "content", message_properties), None
    )
...
## Run the above in a separate cell ##"""
        eval_with_synthetic_code = env.get_template(
            "eval_with_synthetic_example.py"
        ).render({"pipInstall": pip_install_command, "modelUri": model_info.model_uri})
        eval_with_dataset_code = env.get_template(
