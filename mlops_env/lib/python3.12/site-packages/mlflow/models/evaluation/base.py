...
def _resolve_default_evaluator(model_type, default_config) -> list[EvaluatorBundle]:
    ...
    for name in _model_evaluation_registry._registry:
        ...
        if (
            ...
            and evaluator.can_evaluate(
                model_type=model_type, evaluator_config=default_config
            )
        ):
    ...
    if not builtin_evaluators:
        ...
        builtin_evaluators = [
            EvaluatorBundle("default", default_evaluator, default_config)
        ]
...
def resolve_evaluators_and_configs(
    ...
    """
    from mlflow.models.evaluation.evaluator_registry import (
        _model_evaluation_registry as rg,
    )
    ...
    if evaluators is None:
        ...
        for name in evaluators:
            ...
            if evaluator.can_evaluate(model_type=model_type, evaluator_config=config):
                resolved.append(
                    EvaluatorBundle(name=name, evaluator=evaluator, config=config)
                )
        ...
        if evaluators == "default":
            ...
            return [
                EvaluatorBundle(
                    evaluators, rg.get_evaluator(evaluators), evaluator_config
                )
            ]
        else:
            return []
...
def _evaluate(
    *,
    ...
    should_enable_tracing = (
        model is not None
    )  # Do not enable tracing if static dataset is provided
    for eval_ in evaluators:
        ...
        if eval_.evaluator.can_evaluate(
            model_type=model_type, evaluator_config=eval_.config
        ):
            with configure_autologging_for_evaluation(
                enable_tracing=should_enable_tracing
            ):
...
def evaluate(  # noqa: D417
    ...
    *,
    ...
    evaluator_name_to_conf_map = {
        evaluator.name: evaluator.config for evaluator in evaluators
    }
