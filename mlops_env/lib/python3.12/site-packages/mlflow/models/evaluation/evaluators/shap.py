...
class ShapEvaluator(BuiltInEvaluator):
    ...
    def can_evaluate(cls, *, model_type, evaluator_config, **kwargs):
        return model_type in (
            _ModelType.CLASSIFIER,
            _ModelType.REGRESSOR,
    ...
    def _evaluate(
        ...
        **kwargs,
        ...
        if not (
            np.issubdtype(self.y_true.dtype, np.number) or self.y_true.dtype == np.bool_
        ):
        ...
        if self.label_list is None:
            ...
            y_pred = (
                predict_fn(X_df.copy()) if predict_fn else self.dataset.predictions_data
            )
