    ...
    for flavor in FLAVOR_TO_MODULE_NAME:
        ...
        def _setup_autolog(module):
            try:
                ...
                _logger.debug(
                    f"Failed to update autologging config for {flavor}.", exc_info=True
                )
        ...
        try:
            ...
            _logger.debug(
                f"Failed to register post-import hook for {flavor}.", exc_info=True
            )
...
def _should_enable_tracing(flavor: str, autologging_config: dict[str, Any]) -> bool:
    ...
    if global_config.get("disable", False) or (
        not global_config.get("log_traces", True)
    ):
        return False
