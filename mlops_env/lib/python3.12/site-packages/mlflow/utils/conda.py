...
def _get_conda_env_name(conda_env_path, env_id=None, env_root_dir=None):
    ...
    env_name = "mlflow-{}".format(
        hashlib.sha1(
            conda_env_contents.encode("utf-8"), usedforsecurity=False
        ).hexdigest()
    )
    if env_root_dir:
        ...
        env_name += "-{}".format(
            hashlib.sha1(
                env_root_dir.encode("utf-8"), usedforsecurity=False
            ).hexdigest()
        )
...
def get_or_create_conda_env(
    ...
    try:
        ...
        process._exec_cmd(
            [conda_path, "--help"], throw_on_error=False, extra_env=extra_envs
        )
    except OSError:
